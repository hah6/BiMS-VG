...
def get_args_parser():
    ...
    parser.add_argument('--lr_bert', default=0, type=float)#1e-5
    ...
    # Model parameters
    parser.add_argument('--model_name', type=str, default='BIMS-VG',
    ...
    # * Transformer，只有编码器没有解码器，图片经过 CNN(resnet)提取特征后，直接送入 Transformer 编码器。
    ...
    parser.add_argument('--hidden_dim', default=256, type=int,
                        help="Size of the embeddings (dimension of the transformer)")#transformer的输入维度
    ...
    parser.add_argument('--feature_enhancer_layers', default=6, type=int)#增强的层数

    # Transformers in two branches
    parser.add_argument('--bert_enc_num', default=12, type=int)
    parser.add_argument('--detr_enc_num', default=6, type=int)#resnet后的encoder层数
    ...
    parser.add_argument('--vl_enc_layers', default=1, type=int,
                        help='Number of encoders in the vision-language transformer')
    parser.add_argument('--num_points_per_scale',default=[192,96,36], type=list)
    ...
    parser.add_argument('--detr_model', default='./PreDetrParams/detr-r50-unc.pth', type=str, help='detr model')
...
def main(args):
    ...
    print('number of params:', n_parameters)#计算 PyTorch 模型中所有可训练参数（trainable parameters）的总数量
    if args.model_type == "ResNet":#根据模型参数的不同模块进行分组，并为不同组设置不同的学习率
    ...
    # using polynomial lr scheduler or half decay every 10 epochs or step，多种学习率调度策略，根据 args.lr_scheduler 的配置选择不同的学习率衰减方式，用于在训练过程中动态调整学习率。
    if args.lr_scheduler == 'poly':
        ...
        lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, args.lr_drop,gamma=0.1)#lr_drop=60,60epochs后学习率衰减
    else:
        raise ValueError('Lr scheduler type not supportted ')
    ...
    batch_sampler_train = torch.utils.data.BatchSampler(
        sampler_train, args.batch_size, drop_last=True)#用于将 RandomSampler 生成的随机索引按固定批量大小（args.batch_size）分组，并丢弃最后不足一个批量的数据。
    ...
    if args.resume:
         ...
         if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
            ...
            if 'scaler' in checkpoint:  # 加载 scaler 状态
                scaler.load_state_dict(checkpoint['scaler'])

    elif args.detr_model is not None:
        checkpoint = torch.load(args.detr_model, map_location='cpu')# # 加载权重文件
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch == 40:
            print("==> Manually setting lr_bert / lr_visu_cnn / lr_visu_tra to 1e-5 at epoch 40")
            optimizer.param_groups[1]['lr'] = 1e-5
            optimizer.param_groups[2]['lr'] = 1e-5
            optimizer.param_groups[3]['lr'] = 1e-5
        train_stats = train_one_epoch(
            args, model, data_loader_train, optimizer, device, epoch, args.clip_max_norm,scaler=scaler
        )
        lr_scheduler.step()

        val_stats = validate(args, model, data_loader_val, device)

        # 将张量转换为Python原生类型，便于日志记录。
        def tensor_to_serializable(v):
            if isinstance(v, torch.Tensor):
                return v.item() if v.numel() == 1 else v.tolist()
            return v

        log_stats = {
            **{f'train_{k}': tensor_to_serializable(v) for k, v in train_stats.items()},
            **{f'validation_{k}': tensor_to_serializable(v) for k, v in val_stats.items()},
            'epoch': epoch,
        ...
        if args.output_dir:
            ...
            for checkpoint_path in checkpoint_paths:
                utils.save_on_master({
                    ...
                    'scaler': scaler.state_dict()  # 保存 scaler 状态
                }, checkpoint_path)
